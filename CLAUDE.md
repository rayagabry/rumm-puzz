...
- **Per-tier min starting-set count.** `DIFFICULTY_MIN_STARTING_SETS` (hard=0, extra-hard=9) floors the number of sets on the starting board. Extra-hard puzzles can otherwise land at 5–8 sets, which feels sparse — the floor pushes them toward dense boards (more small sets) for visual distraction without changing minMoves. Filter is post-partition, alongside `maxStartingSetSize`.
